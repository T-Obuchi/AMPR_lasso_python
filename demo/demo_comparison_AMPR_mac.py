...
from glmnet import ElasticNet as el
import os
import sys
...
model.lambda_path = np.array([0.2, lambda1])
...
for nexp in np.arange(0, NEXP):
    ...
    # Glmnet
    model.fit(Xmod, Ybs)
    glmnet_ret = model.coef_

    # Recovering original weight
    betaV[w_on, nexp] = w * glmnet_ret[w_on]
    betaV[w_off, nexp] = glmnet_ret[w_off]
